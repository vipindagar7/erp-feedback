...
const fmtSecLabel = (sec, mode = "compact") => {
  ...
  if (mode === "sheet") return [prog, course, sec.semester ? `${sec.semester}Sem` : "", sec.name].filter(Boolean).join(" ");
...
export const exportLevelReport = async ({ level, id, category_id }) => {
  ...
  if (level !== "section") {
    ...
    Object.entries(bySec).forEach(([key, secForms]) => {
      ...
      addSheet((sName0 || secName).slice(0, 31), sheetData, [{ wch: 24 }, { wch: 14 }, { wch: 28 }, { wch: 10 }, { wch: 12 }, { wch: 12 }, { wch: 12 }]);
    });
  }
...
export const exportGroupResults = async (groupId) => {
  ...
  Object.values(bySec).forEach((secForms) => {
    ...
    xlsx.utils.book_append_sheet(wb, ws, (sName1 || `Sec ${sec?.name || "?"}`).slice(0, 31));
  });
